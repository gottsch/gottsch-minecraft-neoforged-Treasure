...
public class SurfaceStructureFeatureGenerator implements IFeatureGenerator {
	...
	public Optional<GeneratorResult<ChestGeneratorData>> generate(IFeatureGenContext context, ICoords spawnCoords,
		...
		Optional<IRuinGenerator<GeneratorResult<ChestGeneratorData>>> ruinGenerator = selectGenerator(context, spawnCoords, rarity);
		if (ruinGenerator.isEmpty()) {
			return Optional.empty();
		}

		// select a template
		// generate structure
		Optional<GeneratorResult<ChestGeneratorData>> ruinResult = ruinGenerator.get().generate(context, spawnCoords, null);
		if (ruinResult.isEmpty()) {
			return Optional.empty();
		}
...
//		generationResult.getData().setPlacement(RegionPlacement.SURFACE);
	 ...
	 * @return
	 */
	public Optional<IRuinGenerator<GeneratorResult<ChestGeneratorData>>> selectGenerator(IWorldGenContext context, ICoords coords, IRarity rarity) {
		List<IRuinGenerator<GeneratorResult<ChestGeneratorData>>> generators = RuinGeneratorRegistry.get(StructureCategory.TERRANEAN);
		return generators.isEmpty() ? Optional.empty() :
			Optional.of(generators.get(context.random().nextInt(generators.size())));
	}
