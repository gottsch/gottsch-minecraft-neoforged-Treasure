...
public class SpawnRuinsCommand {
	...
	private static final SuggestionProvider<CommandSourceStack> SUGGEST_RUIN = (source, builder) -> {
		...
		return SharedSuggestionProvider.suggest(
				TreasureTemplateRegistry.getTemplate(structureCategory.get(), StructureType.RUIN).stream()
	...
	public static int spawn(CommandSourceStack source, BlockPos pos, String category, ResourceLocation name) {
		...
		Treasure.LOGGER.debug("executing t2-ruins command");
